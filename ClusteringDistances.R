...
tictoc::tic()
clust <- kmeans(dat, k#
                #, algorithm = "Lloyd", iter.max = 5000, nstart = 100
                )
tictoc::toc()
beepr::beep()
...
for (i in 1:k) {
  ...
  #Each column name had "X(clusternumber)." at the start of each name
  names(temp) <- substring(names(temp),4)
  temp <- temp %>% select(-clusterID)
  #calculate mahalanobis dist with eacha data's values, cluster's means, and cluster's covariance

...
recluster <- function(input.df, 
                      num.clusters.1 = 4, 
                      num.clusters.2 = 3, 
                      all2 = FALSE,
                      nrep.center.check = 100, 
                      set = TRUE, 
                      seed = 2232021, 
                      first.only = FALSE) {
  print("-------------------------------------------------------------------")
    ...
    if (first.only == FALSE & all2 == TRUE) {
      ...
      for(j in 1:num.clusters.1) {
        ...
        for(n in 2:num.clusters.2) {
          ...
          # layer3[["cluster1.vec"]] <- cluster1.vec
          # temp.list[["cluster2.vec"]] <- cluster2.vec
          # temp.list[["plotdf"]] <- reclusterGathered
...
graph.recluster <- function(input.df, label.vec = NULL, selection1 = NULL, selection2 = NULL, first.only = FALSE) {
  ...
  if (length(selection1) > 0) {
    ...
    ggplot() +   #geom_point(reclusterGathered, mapping = aes(x = as.factor(Construct), y = Value), color = "black",size = 5) + 
      #geom_line(reclusterGathered, mapping = aes(x = as.factor(Construct), y = Value, group = as.factor(cluster)), size = 2) + 

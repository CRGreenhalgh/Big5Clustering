...
grid <- as.data.frame(expand.grid(level1_num_clusters = c(2,4,6,8), level2_num_clusters = c(4,8,16), 
                    num_sd = c(1.5,2,2.5,3,3.5,4))) %>% filter(level2_num_clusters > level1_num_clusters)
...
results <- cbind(grid,"Percent Contained Level 1" = perc.vec1, "Percent Contained Level 2" = perc.vec2)
